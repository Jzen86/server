...
# ISO alpha-2 codes of countries where Gemini API / Google AI Studio is available (from ai.google.dev available-regions doc)
GEMINI_SUPPORTED_CODES=(
...
declare -A CUSTOM_SERVICES_HANDLERS=(
  ...
  [PRIME_VIDEO]="lookup_prime_video"
  [HBO_MAX]="lookup_hbo_max"
  [PARAMOUNT_PLUS]="lookup_paramount_plus"
  [PEACOCK]="lookup_peacock"
  [FUBO_TV]="lookup_fubo_tv"
  [TUBI]="lookup_tubi"
  [CRUNCHYROLL]="lookup_crunchyroll"
  [DAZN]="lookup_dazn"
  [BBC_IPLAYER]="lookup_bbc_iplayer"
  [ABEMA]="lookup_abema"
)
...
parse_arguments() {
  while [[ $# -gt 0 ]]; do
    case $1 in
      ...
      -g | --group)
        case "$2" in
          primary | custom | cdn | all)
            GROUPS_TO_SHOW="$2"
            ;;
          *)
            error_exit "Invalid group: $2. Allowed groups: primary, custom, cdn, all"
            ;;
        esac
        shift 2
        ;;
...
get_asn() {
  ...
  spinner_update "ASN info (IPv$ip_version)"
  log "$LOG_INFO" "Getting ASN info for IPv$ip_version"
  ...
  if [[ -z "$asn" || "$asn" == "null" ]]; then
    log "$LOG_WARN" "ASN info not available for IPv$ip_version"
    return 1
  fi

  if [[ "$ip_version" == "4" ]]; then
    ASN_V4="AS$asn $asn_name"
  else
    ASN_V6="AS$asn $asn_name"
  fi

  log "$LOG_INFO" "ASN info (IPv$ip_version): $([ "$ip_version" == "4" ] && echo "$ASN_V4" || echo "$ASN_V6")"
}
...
redirect_url() {
  local url="$1"
  local ip_version="$2"
  local curl_args=(-s -o /dev/null --location -w '%{http_code}\n%{url_effective}' --max-time "$CURL_TIMEOUT")

  if [[ "$ip_version" == "4" || "$ip_version" == "6" ]]; then
    curl_args+=("-$ip_version")
  fi
...
print_header() {
  ...
  printf "%s: %s\n" "$(color HEADER 'ASN (IPv4)')" "$(bold "${ASN_V4:-N/A}")"

  if [[ -n "$ASN_V6" ]]; then
    printf "%s: %s\n" "$(color HEADER 'ASN (IPv6)')" "$(bold "$ASN_V6")"
  fi
...
lookup_gemini_supported() {
  ...
  if printf "%s\n" "${GEMINI_SUPPORTED_CODES[@]}" | grep_wrapper -Fxq "$country_code"; then
    print_value_or_colored "Yes" "SERVICE"
  else
    print_value_or_colored "No" "HEART"
  fi
}
...
lookup_prime_video() {
  local ip_version="$1"
  local response is_blocked region
  ...
  response=$(curl_wrapper GET "https://www.primevideo.com" \
    --user-agent "$USER_AGENT" \
    --ip-version "$ip_version")
  ...
  is_blocked=$(grep_wrapper -i 'isServiceRestricted' <<<"$response")
  region=$(grep_wrapper --perl '"currentTerritory":"\K[^"]+' <<<"$response" | head -n 1)

  if [[ -z "$is_blocked" && -z "$region" ]]; then
    echo "Failed (PAGE ERROR)"
    return
  fi
  ...
  echo "Failed (Unknown Region)"
}

lookup_hbo_max() {
  local ip_version="$1"
  local response region country_list

  response=$(curl_wrapper GET "https://www.max.com/" \
    --user-agent "$USER_AGENT" \
    --include-headers \
    --ip-version "$ip_version")
  ...
  region=$(grep_wrapper --perl 'countryCode=\K[A-Z]{2}' <<<"$response" | head -n 1)
  country_list=$(grep_wrapper -woP '"url":"/[a-z]{2}/[a-z]{2}"' <<<"$response" | cut -f4 -d'"' | cut -f2 -d'/' | sort -u | xargs | tr 'a-z' 'A-Z')
  country_list="${country_list} US"

  if [[ -z "$region" ]]; then
    echo "Failed (Country Code Not Found)"
    return
  fi

  if grep_wrapper -qw "$region" <<<"$country_list"; then
    echo "Yes (Region: $region)"
  else
    echo "No"
  fi
...
lookup_paramount_plus() {
  local ip_version="$1"
  local result http_code final_url region

  result=$(redirect_url "https://www.paramountplus.com/" "$ip_version")
  ...
  region=$(awk -F'/' '{print $4}' <<<"$final_url" | tr 'a-z' 'A-Z')
  ...
  if [[ "$http_code" == "200" ]]; then
    echo "Yes (Region: ${region:-US})"
    return
  fi

  echo "Failed (Error: $http_code)"
}
...
lookup_peacock() {
  ...
  result=$(redirect_url "https://www.peacocktv.com/" "$ip_version")
  ...
  if grep_wrapper -qi 'unavailable' <<<"$final_url"; then
    echo "No"
    return
  fi
...
lookup_fubo_tv() {
  ...
  if grep_wrapper -qi 'NO_SERVICE_IN_COUNTRY' <<<"$response" ||
    grep_wrapper -q '"network_allowed":false' <<<"$response"; then
    echo "No"
    return
  fi

  if grep_wrapper -q '"network_allowed":true' <<<"$response"; then
    country_code=$(grep_wrapper -oP '"country_code2":"\K[^"]+' <<<"$response")
    echo "Yes (Region: $country_code)"
    return
  fi
...
lookup_tubi() {
  local ip_version="$1"
  local response is_blocked is_ok

  response=$(curl_wrapper GET "https://tubitv.com/home" \
    --user-agent "$USER_AGENT" \
    --ip-version "$ip_version")
  ...
  is_blocked=$(grep_wrapper -i 'not currently available in your area' <<<"$response")
  is_ok=$(grep_wrapper -i 'manifest' <<<"$response")

  if [[ -z "$is_blocked" && -z "$is_ok" ]]; then
    echo "Failed (PAGE ERROR)"
    return
  fi
...
lookup_crunchyroll() {
  ...
  response=$(curl_wrapper GET "https://c.evidon.com/geo/country.js" \
    --user-agent "$USER_AGENT" \
    --ip-version "$ip_version")
  ...
  region=$(grep_wrapper --perl "'code':'\K[a-z]{2}'" <<<"$response" | head -n 1)
...
lookup_dazn() {
  local ip_version="$1"
  local response is_allowed region
  ...
  response=$(curl_wrapper POST "https://startup.core.indazn.com/misl/v5/Startup" \
    --user-agent "$USER_AGENT" \
    --json '{"Version":"2","LandingPageKey":"generic","Languages":"en-US","Platform":"web","Manufacturer":"","PromoCode":"","PlatformAttributes":{}}' \
    --ip-version "$ip_version")
  ...
  if grep_wrapper -qi "Security policy has been breached" <<<"$response"; then
    echo "No (IP Banned)"
    return
  fi

  is_allowed=$(grep_wrapper --perl '"isAllowed"\s{0,}:\s{0,}\K(false|true)' <<<"$response")
  region=$(grep_wrapper --perl '"GeolocatedCountry"\s{0,}:\s{0,}"\K[^"]+' <<<"$response" | tr 'a-z' 'A-Z')

  if [[ "$is_allowed" == "true" ]]; then
    echo "Yes (Region: ${region:-N/A})"
    return
  fi
  ...
  echo "Failed (Error: ${is_allowed:-Unknown})"
}
...
lookup_bbc_iplayer() {
  ...
  response=$(curl_wrapper GET "https://open.live.bbc.co.uk/mediaselector/6/select/version/2.0/mediaset/pc/vpid/bbc_one_london/format/json/jsfunc/JS_callbacks0" \
    --user-agent "$USER_AGENT" \
    --ip-version "$ip_version")
  ...
  is_blocked=$(grep_wrapper -i 'geolocation' <<<"$response")
  is_ok=$(grep_wrapper -i 'vs-hls-push-uk' <<<"$response")
...
lookup_abema() {
  ...
  response=$(curl_wrapper GET "https://api.abema.io/v1/ip/check?device=android" \
    --user-agent "$USER_AGENT" \
    --ip-version "$ip_version")
  ...
  region=$(grep_wrapper --perl '"isoCountryCode"\s{0,}:\s{0,}"\K[^"]+' <<<"$response" | head -n 1)
  ...
  echo "Oversea Only (Region: $region)"
}
