...
# Возврат из альтернативного экрана терминала: восстанавливается то,
# что было на экране до запуска скрипта (история команд пользователя).
restore_screen() {
  printf '\033[?1049l'
}
...
# Приглашение после проверки: «Нажмите 0 для возврата в меню».
wait_return() {
  while true; do
    read -r -p "  Нажмите 0 для возврата в меню: " choice || true
    choice="${choice//$'\r'/}"
...
main() {
  # Переключение на альтернативный экран: вся история пользователя
  # сохраняется в основном буфере и не стирается.
  printf '\033[?1049h'
